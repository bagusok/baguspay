...
const DF_REPLACE_PRODUCT_TITLE = [DF_PRODUCT_BRAND, '-', 'MOBILELEGENDS', 'MOBILELEGEND', 'Go Go']
...
;(async () => {
  ...
  const filterProducts = listProducts.data.filter((product) => {
    return (
      ...
      DF_PRODUCT_TYPE.map((type) => type.toLowerCase()).includes(product.type.toLowerCase())
    )
  })
  ...
  const dbByProviderCode = new Map(productFromDB.map((p) => [p.provider_code, p]))

  const nowListedProviderCodes = new Set(filterProducts.map((p) => p.buyer_sku_code))
  ...
  if (toDisable.length > 0) {
    upserts.push(
      db.update(tb.products).set({ is_available: false }).where(inArray(tb.products.id, toDisable)),
    )
  }
    ...
    JSON.stringify(
      {
        ...
        created: filterProducts.filter((p) => !dbByProviderCode.has(p.buyer_sku_code)).length,
        updated: filterProducts.filter((p) => dbByProviderCode.has(p.buyer_sku_code)).length,
