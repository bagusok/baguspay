...
export const productGroupings = pgTable('product_groupings', {
  ...
  type: productGroupingTypeEnum('type').default(ProductGroupingType.MODAL).notNull(),
...
export const productGroupingRelations = relations(productGroupings, ({ many }) => ({
...
export const productGroupingToProductCategories = pgTable(
  ...
  {
    id: uuid('id').primaryKey().defaultRandom(),
    product_grouping_id: uuid('product_grouping_id').references(() => productGroupings.id, {
      onDelete: 'cascade',
    }),
    product_category_id: uuid('product_category_id').references(() => productCategories.id, {
      onDelete: 'cascade',
    }),
    created_at: timestamp('created_at', {
      withTimezone: true,
