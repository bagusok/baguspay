...
export const paymentSnapshots = pgTable('payment_snapshots', {
  ...
  payment_method_id: uuid('payment_method_id').references(() => paymentMethods.id, {
    onDelete: 'set null',
    onUpdate: 'cascade',
  }),
  type: paymentMethodTypeEnum('type').default(PaymentMethodType.BANK_TRANSFER).notNull(),
  ...
  fee_percentage: numeric('fee_percentage', {
    ...
    .notNull()
    .default(0),
  fee_type: paymentMethodFeeTypeEnum('fee_type').default(PaymentMethodFeeType.MERCHANT),
  ...
  is_need_phone_number: boolean('is_need_phone_number').notNull().default(false),
...
export const paymentSnapshotRelations = relations(paymentSnapshots, ({ one }) => ({
...
export const productSnapshots = pgTable('product_snapshots', {
  ...
  profit_percentage: numeric('profit_percentage', {
    ...
    .default(0),
  provider_code: varchar('provider_code', { length: 50 }).notNull(),
  provider_name: productProviderEnum('provider_name').default(ProductProvider.ATLANTICH2H),
  ...
  provider_input_separator: varchar('provider_input_separator', { length: 3 })
    ...
    .default(''),
  notes: text('notes'),
  billing_type: productBillingTypeEnum('billing_type').default(ProductBillingType.PREPAID),
...
export const productSnapshotRelations = relations(productSnapshots, ({ one }) => ({
