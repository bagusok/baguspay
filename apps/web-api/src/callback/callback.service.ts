...
import { DigiflazzCallbackData } from 'src/integrations/h2h/digiflazz/digiflazz.type'
...
export class CallbackService {
  ...
  async handleTripayCallback(data: TripayCallbackData, callbackSignature: string) {
    ...
    if (data.merchant_ref.startsWith('DEPO')) {
      ...
      if (!order || order.payment_snapshot.provider_name !== PaymentMethodProvider.TRIPAY) {
        throw new BadRequestException('Order not found or not a Tripay order')
      }

      if (order.payment_snapshot.expired_at < new Date()) {
        throw new BadRequestException(`Order with ID ${data.merchant_ref} has expired.`)
      }
  ...
  async handleDigiflazzPrepaidCallback(
    ...
    const { isValid } = this.digiflazzService.verifyCallbackSignature(data, signFromPost)
    ...
    if (!order || order.product_snapshot.provider_name !== ProductProvider.DIGIFLAZZ) {
    ...
    if (data.data.status == 'Sukses') {
      ...
      this.logger.log(`Order ${data.data.ref_id} topup successful. SN: ${data.data.sn}`)
      ...
      this.logger.warn(`Order ${data.data.ref_id} topup failed: ${data.data.message}`)
      ...
      // refund
      if (order.user_id) {
        this.logger.log(`Refunding user ${order.user_id} for failed topup order ${order.order_id}`)
        ...
        this.logger.log(`Refund completed for user ${order.user_id} for order ${order.order_id}`)
      } else {
        this.logger.warn(`No user found for order ${order.order_id}, skipping refund.`)
      }
    } else if (data.data.status == 'Pending') {
      if (event === 'create') {
        this.logger.log(`Order ${data.data.ref_id} is pending. No action taken.`)
      } else if (event === 'update') {
        this.logger.log(`Order ${data.data.ref_id} is still pending. No action taken.`)
      }
    }
  }
}
