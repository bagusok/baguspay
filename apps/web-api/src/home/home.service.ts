...
export class HomeService {
  ...
  public async getBanners() {
    ...
    return SendResponse.success({
      home_top: banners.filter((banner) => banner.banner_location === BannerLocation.HOME_TOP),
  ...
  public async getFastMenus() {
    const fastMenus = await this.databaseService.db.query.productGroupings.findMany({
      columns: {
        id: true,
        name: true,
        image_url: true,
