...
export class TripayApiService {
  ...
  public generateClosedPaymentSignature(merchant_ref: string, amount: number): string {
    return crypto
      .createHmac('sha256', this.configService.get<string>('TRIPAY_PRIVATE_KEY'))
      .update(this.configService.get<string>('TRIPAY_MERCHANT_CODE') + merchant_ref + amount)
      .digest('hex')
  }
  ...
  public async createClosedPayment(data: TripayCreateClosedPaymentRequest) {
    try {
      const response = await this.apiClient.post<TripayCreateClosedPaymentResponse>(
        '/transaction/create',
        data,
      )
      ...
      if (axios.isAxiosError<TripayApiErrorResponse>(error)) {
        ...
        throw new ApiServiceException(error.response.status, error.response.data.message)
      }

      this.logger.error('Unknown error', error)

      throw new ApiServiceException(500, 'Internal server error when connecting to Tripay API')
    }
  }
}
