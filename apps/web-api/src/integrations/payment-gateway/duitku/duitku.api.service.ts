...
export class DuitkuAPiService {
  ...
  public async createTransaction(
    ...
    try {
      const response = await this.API_CLIENT.post<DuitkuCreateTransactionResponseSuccess>(
      ...
      if (isAxiosError(error)) {
        const errorMessage =
          (error.response?.data as { message?: string })?.message || 'Duitku API Error'
        throw new ApiServiceException(
          error.response?.status || HttpStatus.INTERNAL_SERVER_ERROR,
          errorMessage,
        )
      } else {
        throw new ApiServiceException(HttpStatus.INTERNAL_SERVER_ERROR, 'Internal server Error')
      }
    }
  }
