...
export default memo(function PaymentUnavailable({ paymentName, status }: Props) {
  const statusConfig = useMemo(() => {
    switch (status) {
      case 'expired':
        return {
          icon: <ClockIcon className="w-5 h-5 text-gray-600 dark:text-gray-400" />,
      ...
      case 'cancelled':
        return {
          icon: <AlertCircleIcon className="w-5 h-5 text-orange-600 dark:text-orange-400" />,
      ...
      default:
        return {
          icon: <AlertCircleIcon className="w-5 h-5 text-gray-600 dark:text-gray-400" />,
  ...
  return (
    <div className={`p-4 ${statusConfig.bgColor} border ${statusConfig.borderColor} rounded-lg`}>
      <div className="flex items-center justify-center gap-2 mb-2">
        {statusConfig.icon}
        <p className={`text-sm ${statusConfig.textColor} font-medium`}>{statusConfig.title}</p>
      </div>
      <p className={`text-sm ${statusConfig.textColor} text-center`}>{statusConfig.message}</p>
    </div>
  )
})
