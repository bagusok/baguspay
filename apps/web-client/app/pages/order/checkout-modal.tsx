...
export default function CheckoutModal({ data }: Props) {
  ...
  const checkout = useMutation<any, Error, string | undefined>({
    ...
    mutationFn: async (pinOverride?: string) => {
      ...
      const pinToUse = pinOverride ?? balancePin
      if (selectedPayment.id === balancePaymentMethod.data?.data?.id && pinToUse) {
        payload.pin = pinToUse
      }
  ...
  // Clear PIN when switching away from balance method
  useEffect(() => {
    if (selectedPayment?.id !== balancePaymentMethod.data?.data?.id) {
      setBalancePin('')
      setIsPinDialogOpen(false)
    }
  }, [selectedPayment?.id, balancePaymentMethod.data?.data?.id])
  ...
  const handleCheckout = () => {
    ...
    checkout.mutate(balancePin || undefined)
  }
