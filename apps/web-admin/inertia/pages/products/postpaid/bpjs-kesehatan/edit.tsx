...
export default function EditProductCategory(
  props: InferPageProps<ProductsCategoriesPostpaidController, 'editBPJSKesehatan'>
) {
  const { data, errors, setData, patch, processing } = useForm('editbpjskesehatan', {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault()
    patch(`/admin/product-categories/bpjs-kesehatan/${props.productCategory.id}`, {
      onSuccess: () => {
  ...
  return (
    <AdminLayout>
      <h2 className="text-2xl font-bold mt-4">Edit BPJS Kesehatan Product Category</h2>
