...
export default function ProductCategory(props: Props) {
  ...
  const handleSearch = (e: React.FormEvent) => {
    e.preventDefault()
    router.get('/admin/product-categories/postpaid/tagihan-pln', { searchBy, searchQuery })
  }

  const handlePageChange = (page: number) => {
    router.get('/admin/product-categories/postpaid/tagihan-pln', { ...filters, page })
  }

  return (
    <AdminLayout>
      <div className="flex justify-between mt-5 mb-2">
        <h1 className="text-2xl font-bold">List Voucher</h1>
        <Button asChild>
          <Link href="/admin/product-categories/postpaid/tagihan-pln/create">Add New</Link>
        </Button>
      </div>
    ...
    cell: ({ row }) => (
      <div className="flex space-x-2" key={row.original.id}>
        <Button variant="outline" size="sm" asChild>
          <Link href={`/admin/product-categories/postpaid/tagihan-pln/${row.original.id}`}>
            Detail
          </Link>
        </Button>
        <Button variant="secondary" size="sm" asChild>
          <Link href={`/admin/product-categories/postpaid/tagihan-pln/${row.original.id}/edit`}>
            Edit
          </Link>
        </Button>
        <Dialog>
          ...
          <DialogContent>
            ...
            <DialogFooter>
              <Button
                variant="destructive"
                onClick={() => {
                  router.delete(`/admin/product-categories/tagihan-pln/${row.original.id}`, {
                    preserveScroll: true,
                    onSuccess: () => {
                      router.get('/admin/product-categories/postpaid/tagihan-pln')
                    },
                  })
                }}
              >
