...
export default function ProductCategory(props: Props) {
  ...
  const handleSearch = (e: React.FormEvent) => {
    e.preventDefault()
    router.get('/admin/product-categories/kuota', { searchBy, searchQuery })
  }

  const handlePageChange = (page: number) => {
    router.get('/admin/product-categories/kuota', { ...filters, page })
  }
    ...
    cell: ({ row }) => (
      <div className="flex space-x-2" key={row.original.id}>
        <Button variant="outline" size="sm" asChild>
          <Link href={`/admin/product-categories/kuota/${row.original.id}`}>Detail</Link>
        </Button>
        <Button variant="secondary" size="sm" asChild>
          <Link href={`/admin/product-categories/kuota/${row.original.id}/edit`}>Edit</Link>
        </Button>
        <Dialog>
          ...
          <DialogContent>
            ...
            <DialogFooter>
              <Button
                variant="destructive"
                onClick={() => {
                  router.delete(`/admin/product-categories/kuota/${row.original.id}`, {
                    preserveScroll: true,
                    onSuccess: () => {
                      router.get('/admin/product-categories/kuota')
                    },
                  })
                }}
              >
