...
export default function ProductCategory(props: Props) {
  ...
  const handleSearch = (e: React.FormEvent) => {
    e.preventDefault()
    router.get('/admin/product-categories/e-wallet', { searchBy, searchQuery })
  }

  const handlePageChange = (page: number) => {
    router.get('/admin/product-categories/e-wallet', { ...filters, page })
  }

  return (
    <AdminLayout>
      <div className="flex justify-between mt-5 mb-2">
        <h1 className="text-2xl font-bold">List E-Wallet</h1>
        <Button asChild>
          <Link href="/admin/product-categories/e-wallet/create">Add New</Link>
        </Button>
      </div>
