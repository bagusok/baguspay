...
type Props = InferPageProps<OrdersController, 'index'>
...
const columns: ColumnDef<Props['orders'][number]>[] = [
  ...
  {
    accessorKey: 'product_snapshot.billing_type',
    header: 'Type',
    cell: ({ row }) => {
      const billingType = row.original.product_snapshot.billing_type
      return (
        <span
          className={cn(
...
export default function OrderIndex(props: Props) {
  ...
  const [billingType, setBillingType] = useState(filters.billingType || undefined)
  ...
  const handlePageChange = (page: number) => {
    router.get('/admin/orders', { ...filters, page, limit })
  }

  const handleBillingTypeQuickFilter = (type: ProductBillingType | undefined) => {
    setBillingType(type)
    router.get('/admin/orders', {
      ...filters,
      billingType: type,
      ...
      <div className="flex justify-between mt-5 mb-2">
        ...
        <div className="flex items-center gap-2">
          <div className="flex gap-1">
            <Button
              variant={!billingType ? 'default' : 'outline'}
              size="sm"
              onClick={() => handleBillingTypeQuickFilter(undefined)}
            >
              All
            </Button>
            <Button
              variant={billingType === ProductBillingType.PREPAID ? 'default' : 'outline'}
              size="sm"
              onClick={() => handleBillingTypeQuickFilter(ProductBillingType.PREPAID)}
            >
              Prepaid
            </Button>
            <Button
              variant={billingType === ProductBillingType.POSTPAID ? 'default' : 'outline'}
              size="sm"
              onClick={() => handleBillingTypeQuickFilter(ProductBillingType.POSTPAID)}
            >
              Postpaid
            </Button>
          </div>
            ...
            <DialogContent>
              ...
              <form onSubmit={handleSearch} className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    <Label className="mb-1">Billing Type</Label>
                    <Select
                      onValueChange={(v) => setBillingType(v as ProductBillingType)}
                      value={billingType}
                    >
                      ...
                      <SelectContent>
                        {Object.values(ProductBillingType).map((type) => (
                          <SelectItem key={type} value={type}>
                            {type === ProductBillingType.PREPAID ? 'Prepaid' : 'Postpaid'}
                          </SelectItem>
                        ))}
                      </SelectContent>
                    </Select>
