...
type Props = InferPageProps<OfferController, 'indexFlashSale'>
    ...
    cell: ({ row }) => (
      <div className="flex space-x-2">
        <Button size="sm" asChild>
          <Link href={`/admin/offers/flash-sale/${row.getValue('id')}/edit`}>Edit</Link>
        </Button>
        <Button
          variant="destructive"
          size="sm"
          onClick={() => {
            ...
            router.delete(`/admin/offers/${offerId}`, {
              preserveScroll: true,
              onSuccess: () => {
                router.get('/admin/offers/flash-sale')
              },
            })
          }}
        >
...
export default function OfferIndex(props: Props) {
  ...
  const handleSearch = (e: React.FormEvent) => {
    e.preventDefault()
    router.get('/admin/offers/flash-sale', { searchBy, searchQuery })
  }

  const handlePageChange = (page: number) => {
    router.get('/admin/offers/flash-sale', { ...filters, page })
  }
  ...
  return (
    <AdminLayout>
      <div className="flex justify-between mt-5 mb-2 items-start">
        ...
        <Button asChild>
          <Link href="/admin/offers/flash-sale/create">Create Flash Sale</Link>
        </Button>
      </div>
